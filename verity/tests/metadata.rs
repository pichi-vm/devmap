        ...
        for data_order in 9..=19 {
            for hash_order in 9..=19 {
                for hash_type in [0u32, 1] {
                    ...
                    assert_eq!(hashes.header().data_block_size().get(), 1 << data_order);
                    assert_eq!(hashes.header().hash_block_size().get(), 1 << hash_order);
                    assert_eq!(
                        hashes.header().hash_type().to_string(),
                        hash_type.to_string()
                    );
                }
            }
...
fn salt_boundaries_and_largest_addressable_data_extent_are_valid() {
    for size in [0u16, 256] {
        ...
        let hashes = Hashes::open(Cursor::new(bytes)).unwrap();
        assert_eq!(hashes.header().salt(), vec![0xff; usize::from(size)]);
    }
}
