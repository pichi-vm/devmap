...
//! dm-verity v1 on-disk format: the hash tree the kernel walks, plus the
//! cryptsetup-compatible `verity_sb` superblock that userspace reads to
//! reconstruct the kernel table line.
//!
//! Output is byte-identical to `veritysetup format` — [`VerityBuilder`] is
//! cross-validated against it (see `matches_veritysetup_format_multiblock`)
//! — so trees built here interoperate with cryptsetup and the kernel.
//! Building is deterministic: the same `(data, params)` yield the same
//! `(blob, root_hash)`.
//!
//! This crate is pure computation. It never opens `/dev/mapper/control`
//! and does not depend on `devmap-linux`; activating a verity device from
//! a tree built here is the caller's job (via `devmap-linux` or
//! `veritysetup`).
//!
//! Two format owners meet here. The hash **tree** is the kernel's format —
//! it reads it on every block access. The `verity\0\0` **superblock** is
//! cryptsetup's — the kernel never reads it; it exists only so userspace
//! can rebuild the table line. Both are emitted together for veritysetup
//! interop; the `write_superblock` helper is the separable superblock half.
//!
//! All multi-byte superblock fields are little-endian.

// Lint posture for a byte-format builder. The dm-verity encoding is
...
// blob is cross-checked byte-for-byte against `veritysetup format`, so the
// cast lints are noise. The panics are internal invariants (`expect` on a
// non-empty level), not caller-triggerable, so a `# Panics` section would
// mislead. Format docs name many bare identifiers (SHA256, NUL, TOP-DOWN)
// where backticks add nothing.
#![allow(
...
/// Errors produced by [`VerityParams::compute`] and the streaming
/// [`VerityBuilder`]. Public because both are part of the crate API, so
/// the typed error surface is part of the contract.
#[derive(Debug, Error)]
pub enum VerityError {
    ...
    /// `compute()` would spin forever. Callers that lock
    /// `hash_block_size = 4096` never reach this, but `VerityParams` is
    /// public and other callers pick block sizes freely.
    #[error(
...
/// cryptsetup's `verity_sb` signature.
pub const VERITY_SIGNATURE: &[u8; 8] = b"verity\0\0";
/// Total size of `struct verity_sb`.
...
/// "normal" hash type (Chrome OS would be 0).
...
/// Verity construction parameters. Typical defaults:
...
pub struct VerityParams {
    /// `verity_sb.data_block_size`. The data byte length passed to
    ...
    /// zero prefix plus optional author-supplied suffix.
    /// Length MUST be ≤ 256.
    pub salt: Vec<u8>,
    /// `verity_sb.uuid`. Deterministic:
    /// `SHA256(salt || data_digest)[..16]`.
    pub uuid: [u8; 16],
}

impl VerityParams {
    /// Check the [`VerityParams`] preconditions documented above. Both
    /// `compute` and `VerityBuilder::new` call this
    ...
    pub fn validate(&self) -> Result<(), VerityError> {
        ...
        // Guard the infinite-loop case: a hash block must hold at
...
/// Streaming hash-tree builder.
///
/// Lets callers feed the COW blob one [`VerityParams::data_block_size`]
/// chunk at a time without ever materialising the full data as a `Vec<u8>`.
/// Internally accumulates only the leaf hashes (32 bytes per data block,
/// i.e. ~0.78% of the data size for SHA-256 + 4 KiB blocks) until
...
/// in-memory inputs (tests, small fixtures); production callers drive the
/// builder directly from a streaming read of the data device.
///
/// # Memory profile (for a 10 GiB data at default 4 KiB data blocks)
///
...
impl VerityBuilder {
    ...
    pub fn add_data_block(&mut self, block: &[u8]) -> Result<(), VerityBuilderError> {
        ...
        // Zero-pad short blocks (hashing operates over
    ...
    pub fn finalize(self) -> VerityOutput {
        ...
        // + the root block is `salt || top_level_packed_into_one_block`
        ...
        //
        // Trap V6b: the previous
        // condition `> 1` over-built the tree by one level for any input
        // where `1 < leaf_count <= hash_per_block` (e.g. a 100-block data at
        ...
        // regression coverage.
        //
        // Edge case: 0 data blocks (empty data) — degenerate, the leaf level
...
/// data block at a time — the single per-block read loop for streaming
/// callers.
///
...
///
/// Partial-final-block correctness is handled by
/// [`VerityBuilder::add_data_block`] itself, which zero-pads a short block
/// internally; this helper does NOT pad. Adding a padding loop here would
/// double-zero-pad and break byte-equivalence with `veritysetup format`.
///
...
///
/// # Veritysetup cross-validation
///
/// `feed_from_reader == compute` is asserted by the streaming-equivalence
/// tests, and `compute() == veritysetup format` byte-for-byte by
/// `matches_veritysetup_format_multiblock`. Composing the two gives the
...
pub struct VerityOutput {
    ...
    /// blob; it is the value `veritysetup verify` and the kernel verity
    /// table line consume.
...
/// Build the dm-verity v1 hash tree blob over `data`.
///
/// # Algorithm (per )
///
/// 1. Slice `data` into `data_block_size`-byte data blocks.
...
///    levels written TOP-DOWN (root level first; trap V7).
...
impl VerityParams {
    /// Compute the dm-verity tree for an in-memory data (tests, small fixtures).
    /// Production callers that stream the data from disk should drive
    /// [`VerityBuilder`] directly to avoid materialising the full data.
    ///
    /// Hard-validates the params: a future caller that picks non-default
    /// params must not spin forever (hash_per_block < 2) or divide by zero
    /// (block size 0).
    pub fn compute(&self, data: &[u8]) -> Result<VerityOutput, VerityError> {
        let mut builder = VerityBuilder::new(self)?;
        let dbs = self.data_block_size as usize;
        for chunk in data.chunks(dbs) {
            builder
...
/// `hashes_per_block` trap V1: rounded-DOWN power of two.
///
...
///
/// For a 4 KiB data (1 data block) → returns 0 (no parent levels needed).
/// For 128 data blocks → returns 1 (one root block holds all 128 leaf hashes).
///
/// Test-only: `VerityBuilder::finalize` walks levels
...
/// `HASH(salt || data_block)` — dm-verity v1 salt-prepend construction.
fn hash_v1(salt: &[u8], block: &[u8]) -> [u8; SHA256_DIGEST_SIZE] {
    let mut h = Sha256::new();
    h.update(salt); // PREPEND for v1
    h.update(block);
    let full = h.finalize();
...
/// Write the 512-byte verity_sb at the start of `blob`
...
fn write_superblock(blob: &mut Vec<u8>, params: &VerityParams, data_blocks: u64) {
    ...
    // signature[8] — trap (no number, but verity.c:22 fixed).
    ...
    // uuid[16] — deterministic.
    blob.extend_from_slice(&params.uuid);

    // algorithm[32] — trap V4: lowercase ASCII, NUL-padded.
    ...
    // data_blocks (u64 LE) — trap V5: in BLOCKS, not bytes.
    blob.extend_from_slice(&data_blocks.to_le_bytes());

    // salt_size (u16 LE) — trap V3: actual length of salt
...
mod tests {
    ...
    /// trap V1: hashes_per_block is bit-shift form, NOT
    ...
    /// trap V2: v1 hash construction is salt-PREPEND.
    ...
    /// + traps V4, V5: superblock byte layout.
    #[test]
    ...
    /// trap V7 + lines 383–393: tree-level math.
    ///
    ...
    ///: root_hash is deterministic
    /// for the same `(data, params)`. Two calls produce the same
    /// blob and same root_hash.
    #[test]
    fn root_hash_deterministic_for_known_fixture() {
        let salt = vec![0u8; 32];
        let data_digest = [0x42u8; 32];
        let uuid = VerityParams::derive_uuid(&salt, &data_digest);
        let params = VerityParams {
            data_block_size: 4096,
            hash_block_size: 4096,
            salt,
        ...
        let data = vec![0u8; 16 * 1024];
        let a = params.compute(&data).unwrap();
        let b = params.compute(&data).unwrap();
    ...
    /// hash_block_size = digest_size (32) makes hashes_per_block
    ...
    fn validate_rejects_hash_block_equal_to_digest_size() {
        ...
        let err2 = params.compute(&data).unwrap_err();
        assert!(matches!(err2, VerityError::HashBlockTooSmall { .. }));
    }

    /// hash_block_size < digest_size also makes hashes_per_block
    ...
    /// `VerityBuilder` produces the same `(blob, root_hash)` as
    ...
    fn streaming_builder_matches_compute() {
        ...
        // 16 KiB data → 4 data blocks (4096 each) + 1 short block at the end.
        let data: Vec<u8> = (0u32..)
            .map(|i| (i & 0xFF) as u8)
            .take(16 * 1024 + 257)
            .collect();
        let in_mem = params.compute(&data).unwrap();

        let mut builder = VerityBuilder::new(&params).unwrap();
        for chunk in data.chunks(params.data_block_size as usize) {
    ...
    /// `VerityBuilder::add_data_block` rejects subsequent calls
    ...
    /// `VerityBuilder::add_data_block` rejects a block larger
    ...
    /// `VerityBuilder::new` propagates the same `VerityError` as
    /// `compute` for invalid params.
    #[test]
    fn streaming_builder_validates_params() {
        let params = VerityParams {
            data_block_size: 4096,
            hash_block_size: 16, // too small
            salt: vec![0u8; 32],
            uuid: [0u8; 16],
        };
    ...
    /// Regression for the cross-validation gap (trap V6b in the
    ...
    /// leaf hash).
    ///
    /// The other tests (streaming-equivalence, deterministic
    ...
    ///
    /// 100 data blocks × 4 KiB = 400 KiB data; with hash_per_block = 128
    ...
    /// `feed_from_reader` produces byte-identical output
    /// to `compute()` for a buffer whose length is exactly a multiple of
    /// `data_block_size`. Composes with the CI cross-check (which
    ...
    /// blocks plus a sub-block tail). the partial-final-block rule — proves the
    ...
    /// `feed_from_reader` handles empty input — no
    ...
    /// `validate` accepts the typical 4 KiB defaults.
    #[test]
    ...
    /// non-zero salt + derived UUID. This is the load-bearing guarantee
    /// that lets this crate and `veritysetup` produce interoperable
    /// images. Skips when veritysetup is absent.
    #[test]
