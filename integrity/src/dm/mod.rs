...
/// [`Target`]'s write mode.
///
/// Uses the single-character table tokens `D`, `J`, `B`, `R`, and `I`.
/// Parsing rejects other tokens.
...
impl std::fmt::Display for InitializationFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{}; failed to remove temporary device {}: {}",
            self.primary, self.name, self.cleanup
        )
    }
}
...
impl Target {
    /// Start building an [`Target`]. `tag_size`/`internal_hash`
    ...
    /// Initializes the backing device through a temporary one-sector kernel table.
    ///
    /// Clears and persists the superblock first, loads and resumes the temporary
    /// device, then requests its removal and reads the resulting usable capacity.
    /// The supplied path must identify this target's backing device.
    ///
    /// # Errors
    ///
    /// Returns a storage or backend error. Failure may leave the backing device
    /// partially initialized. On load/resume failure, removal is attempted;
    /// if cleanup also fails, the error reports that the temporary device remains.
    pub fn format(
        &self,
        control: &impl devmap_core::Control,
        name: &str,
        backing: impl AsRef<std::path::Path>,
    ) -> io::Result<u64> {
        use devmap_core::{Device as _, TableBuilder as _};
        ...
        let mut file = std::fs::OpenOptions::new()
            ...
            .write(true)
            .open(backing)?;
        std::io::copy(&mut std::io::repeat(0).take(4096), &mut file)?;
        file.flush()?;
        file.sync_all()?;
        // Closing the zeroing handle invalidates its buffered block-device
        // pages before the kernel writes the superblock through its own I/O.
        drop(file);
        let device = control.create(name)?;
        let prepared = device
            .builder()
            .add(0, 1, self.clone())
            .and_then(devmap_core::TableBuilder::load)
            .and_then(|()| device.resume());
        if let Err(error) = prepared {
            return match device.remove_deferred() {
                Ok(()) => Err(error),
        ...
        Ok(crate::Header::open(std::fs::File::open(backing)?)?.data_sectors())
    }
}
...
///
/// The read-back shape differs from the construction parameters. The kernel
/// answers `DM_TABLE_STATUS` with its whole effective
...
/// device. None of that fits [`Target`], whose fields describe what a
/// caller can ask for.
///
...
impl FromStr for Table {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let reserved_sectors = fields.next().ok_or(ParseError)?.parse()?;
        let tag_size = fields.next().ok_or(ParseError)?.parse()?;
        let mode = fields.next().ok_or(ParseError)?.parse()?;
        ...
        for _ in 0..count {
            ...
            match key {
                "meta_device" => {
                    table.meta_device = Some(value.ok_or(ParseError)?.parse()?);
                }
                "block_size" => table.block_size = Some(value.ok_or(ParseError)?.parse()?),
                ...
                "interleave_sectors" => {
                    table.interleave_sectors = Some(value.ok_or(ParseError)?.parse()?);
                }
                "buffer_sectors" => table.buffer_sectors = value.ok_or(ParseError)?.parse()?,
                "journal_sectors" => {
                    table.journal_sectors = Some(value.ok_or(ParseError)?.parse()?);
                }
                "journal_watermark" => {
                    table.journal_watermark_percent = Some(value.ok_or(ParseError)?.parse()?);
                }
                "commit_time" => table.commit_time_ms = Some(value.ok_or(ParseError)?.parse()?),
                "sectors_per_bit" => {
                    table.sectors_per_bit = Some(value.ok_or(ParseError)?.parse()?);
                }
                "bitmap_flush_interval" => {
                    table.bitmap_flush_interval_ms = Some(value.ok_or(ParseError)?.parse()?);
                }
...
/// [`Target`]'s runtime status: how many integrity checks have failed,
/// the usable capacity, and any recalculation in flight.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub struct Info {
    /// Target check failures detected since the device was loaded.
...
impl FromStr for Info {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let provided_data_sectors = fields.next().ok_or(ParseError)?.parse()?;
        let recalc_sector = match fields.next().ok_or(ParseError)? {
            "-" => None,
...
mod tests {
    ...
    fn integrity_renders_dash_for_unset_tag_size() {
        ...
        let t = Target::builder(DevId::new(252, 1).unwrap(), 0, Mode::Journaled)
    ...
    fn integrity_renders_optional_args() {
        let t = Target::builder(DevId::new(252, 1).unwrap(), 0, Mode::Direct)
            .tag_size(32)
            .internal_hash("sha256")
            .allow_discards(true)
            .build();
    ...
    fn integrity_renders_mode_chars() {
        let bitmap = Target::builder(DevId::new(252, 1).unwrap(), 0, Mode::Bitmap)
            .internal_hash("sha256")
            .build();
        assert!(line(0, 8192, &bitmap).contains(" B "));
        let recovery = Target::builder(DevId::new(252, 1).unwrap(), 0, Mode::Recovery)
            .internal_hash("sha256")
            .build();
        assert!(line(0, 8192, &recovery).contains(" R "));
        let inline = Target::builder(DevId::new(252, 1).unwrap(), 0, Mode::Inline)
            .internal_hash("sha256")
            .build();
    ...
    fn table_holds_what_integrity_cannot() {
        ...
        // lose the answer.
        let written = Target::builder(DevId::new(7, 0).unwrap(), 0, Mode::Journaled)
            .internal_hash("sha256")
            .build();
