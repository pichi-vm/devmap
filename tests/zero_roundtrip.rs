...
fn create_load_resume_read_zeros_remove() {
    ...
    let mut file =
        std::fs::File::open(&path).unwrap_or_else(|e| panic!("open {path} (major={major}, minor={minor}): {e}"));

    let mut buf = [0xFFu8; 4096];
    file.read_exact(&mut buf).expect("read from dm-zero device");
    assert!(buf.iter().all(|&b| b == 0), "dm-zero must read back as all zeros");
...
fn suspend_resume_round_trips() {
    ...
    assert!(!status.is_suspended(), "device should not be suspended after resume()");

    removed.suspend().expect("DM_DEV_SUSPEND (suspend)");
    let status = removed.status().expect("DM_DEV_STATUS");
    assert!(status.is_suspended(), "device should be suspended after suspend()");

    removed.resume().expect("DM_DEV_SUSPEND (resume again)");
    let status = removed.status().expect("DM_DEV_STATUS");
    assert!(!status.is_suspended(), "device should not be suspended after resuming again");
}
...
fn by_device_and_by_node_attach_to_an_existing_device() {
    ...
    assert_eq!(by_device.status().expect("DM_DEV_STATUS via by_device").target_count(), 1);
...
fn create_rejects_a_duplicate_name() {
    ...
    let err = control.create(&name).expect_err("creating the same name twice must fail");
    assert!(matches!(err, devmap::Error::NameConflict { .. }), "expected NameConflict, got {err:?}");
}
