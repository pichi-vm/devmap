...
//! dropping one does nothing, because a dm device is kernel state that
//! outlives the process. Devices are identified by a [`DevId`]
//! (`major:minor`).
//!
//! Teardown is therefore always explicit, and there is no `Drop`-based
//! autoremoval here by design. [`Device::remove`] removes immediately and
//! reports `EBUSY` if the device is open; [`Device::remove_deferred`] hands
//! the job to the kernel, which removes the device once its last holder
//! closes it. The second is the only autoremoval on offer, and it is the
//! only one that still works when this process is killed.
//!
...
//!
//! dev.remove()?;                                // nothing implicit about it
//! # Ok(())
//! # }
//! ```
...
pub use device::{DevId, Device, LiveTarget, Status};
