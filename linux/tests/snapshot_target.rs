...
fn the_kernel_serves_an_image_from_a_store_this_crate_wrote() {
    ...
    let work_cow_path = temporary("work-cow");
    let cow_path = temporary("cow");
    std::fs::write(&raw_path, &contents).expect("write the raw image");

    // Compose the image in an oversized work COW, then emit the exact-size
    // artifact that the kernel will consume.
    let mut raw = File::open(&raw_path).expect("open the raw image");
    let mut cow_file = OpenOptions::new()
        ...
        .truncate(true)
        .open(&work_cow_path)
        .expect("create the work store");
    cow_file
        .set_len((u64::try_from(chunks).expect("chunk count fits") + 2) * SIZE as u64)
        .expect("size the work store");
    let mut layer =
        Layer::create(Zero::new(length as u64), &mut cow_file, CHUNK_SIZE).expect("create layer");
    std::io::copy(&mut raw, &mut layer).expect("copy image");
    let mut compact = OpenOptions::new()
        .write(true)
        .create(true)
        .truncate(true)
        .open(&cow_path)
        .expect("create compact store");
    layer.compact(&mut compact).expect("compact the store");
    compact.sync_all().expect("persist compact store");
    drop(layer);
    drop(cow_file);
    drop(compact);
    std::fs::remove_file(&raw_path).ok();
    std::fs::remove_file(&work_cow_path).ok();

    assert_eq!(
        std::fs::metadata(&cow_path)
            .expect("stat compact store")
            .len(),
        6 * SIZE as u64,
        "header, metadata, and four retained data chunks"
    );
...
fn the_kernel_accepts_a_store_this_crate_merged_down() {
    ...
    // Write a layer over the origin, then merge it straight back down.
    {
        ...
        cow_file
            .set_len((chunks + 2) * SIZE as u64)
            .expect("size the store");

        let mut layer =
            Layer::create(&mut origin_file, cow_file, CHUNK_SIZE).expect("create layer");
        for &(index, byte) in marked {
            layer
        ...
        .expect("open layer");
        layer.merge().expect("merge the store down");
    }
