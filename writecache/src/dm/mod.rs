...
/// Backing store kind for a [`Target`] cache device.
///
/// Encoded as `s` for SSD storage or `p` for persistent memory. Parsing rejects
/// other tokens.
...
impl Target {
    /// Start building a [`Target`]. The watermark options default to
...
impl fmt::Display for Target {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "{} {} {} {}",
            self.kind, self.origin, self.cache, self.block_size
        )?;
        let opt_count = 2
...
impl FromStr for Target {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let kind = fields.next().ok_or(ParseError)?.parse()?;
        let origin = fields.next().ok_or(ParseError)?.parse::<DevId>()?;
        let cache = fields.next().ok_or(ParseError)?.parse::<DevId>()?;
        let block_size = fields.next().ok_or(ParseError)?.parse()?;
        ...
        while consumed < count {
            match fields.next().ok_or(ParseError)? {
                "high_watermark" => {
                    high_watermark_percent = Some(fields.next().ok_or(ParseError)?.parse()?);
                }
                "low_watermark" => {
                    low_watermark_percent = Some(fields.next().ok_or(ParseError)?.parse()?);
                }
...
/// [`Target`]'s runtime status: cache occupancy plus fourteen
...
impl FromStr for Info {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let has_error = fields.next().ok_or(ParseError)?.parse::<i64>()? != 0;
        let info = Info {
            has_error,
            blocks: fields.next().ok_or(ParseError)?.parse()?,
            free_blocks: fields.next().ok_or(ParseError)?.parse()?,
            writeback_blocks: fields.next().ok_or(ParseError)?.parse()?,
            reads: fields.next().ok_or(ParseError)?.parse()?,
            read_hits: fields.next().ok_or(ParseError)?.parse()?,
            writes: fields.next().ok_or(ParseError)?.parse()?,
            write_hits_uncommitted: fields.next().ok_or(ParseError)?.parse()?,
            write_hits_committed: fields.next().ok_or(ParseError)?.parse()?,
            writes_around: fields.next().ok_or(ParseError)?.parse()?,
            writes_allocate: fields.next().ok_or(ParseError)?.parse()?,
            writes_blocked_on_freelist: fields.next().ok_or(ParseError)?.parse()?,
            flushes: fields.next().ok_or(ParseError)?.parse()?,
            discards: fields.next().ok_or(ParseError)?.parse()?,
        };
        if fields.next().is_some() {
            return Err(ParseError);
        }
...
pub trait Commands: TargetEndpoint<Target = Target> {
    ...
    fn flush(&self) -> io::Result<()> {
        self.message("flush").map(drop)
    }

    /// `flush_on_suspend` — arm a flush to run on the next suspend.
    fn flush_on_suspend(&self) -> io::Result<()> {
        self.message("flush_on_suspend").map(drop)
    }

    /// `cleaner` — enter cleaner mode: flush everything and stop caching,
    /// so the cache can be detached.
    fn cleaner(&self) -> io::Result<()> {
        self.message("cleaner").map(drop)
    }

    /// `clear_stats` — reset the hit/miss counters
    /// the backend's status query reports.
    fn clear_stats(&self) -> io::Result<()> {
        self.message("clear_stats").map(drop)
    }
}
...
mod tests {
    ...
    fn writecache_from_str_rejects_unmodelled_options_and_modes() {
        ...
        assert!("x 252:1 252:2 4096 0".parse::<Target>().is_err());
    }
}
